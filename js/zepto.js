...
class Dialog {
    ...
    init(obj){
      ...
      let btnStr = '';
      let dialogClass = '';
      switch (me.dialogType) {
      	case 'promit': 
      			dialogClass = 'dialog-promit';
      			btnStr = '<a class="dialog-yes J-yes">是</a><a class="dialog-no J-no">否</a>'; break;
      	case 'alert' : 
      			dialogClass = 'dialog-alert';
      			btnStr = `<a class="dialog-ok J-sure">${obj.sureText?obj.sureText:"OK"}</a>`; break;
      	default: 
      			dialogClass = 'dialog-msg';
      			btnStr = '';
      }
      me.content = '<div class="' + dialogClass + '">' + obj.content  + '</div>' + btnStr;
      let html = `<div class="dialog-shadow dialog-wrap"></div>
                  <div class="dialog-content translate dialog-wrap">
                    ${me.content}
                  </div>`;
      $('body').append(html);

      $('.dialog-shadow, .J-yes, .J-sure').on('click', function(event) {
        event.preventDefault();
        /* Act on the event */
        if ($(this).hasClass('J-sure') && obj.sureFn) {
        	obj.sureFn && obj.sureFn()
        } else {
        	me.closeDialog();
        }
      });
      if(me.dialogType == 'msg'){
      	setTimeout(() => {
      		me.closeDialog();
      	}, 2000)
      }
      $('.J-no').click(function(event) {
      	/* Act on the event */
      	obj.btnNo && obj.btnNo()
      });
    }
